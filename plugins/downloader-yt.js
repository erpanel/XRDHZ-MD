...
let handler = async (m, { conn, args, command }) => {
  try {
    switch (command) {
      case 'ytmp4': {
        if (!args[0]) return m.reply(`*Contoh :* .${command} https://youtu.be/JiEW1agPqNY?si=OUpQ4GCaQpLKTL0H 360p`)
        let f = args[1] || '360p'
      ...
      case 'ytmp3': {
        if (!args[0]) return m.reply(`*Contoh :* .${command} https://youtu.be/JiEW1agPqNY?si=OUpQ4GCaQpLKTL0H`)
        let f = args[1] || '128k'
        let { buffer, fileName } = await yt.download(args[0], f)
